...
#
    def guest_arrival(self, *guests):
        ...
        list_of_tables = self.tables
        len_list_of_guests = len(list_of_guests)
        min_guests_tables = min(len_list_of_guests, len(self.tables))

        for i in range(min_guests_tables):
            list_of_tables[i].guest = guests[i]
            thr1 = guests[i]
            thr1.start()
            Cafe.list_thread.append(thr1)
            print(f'{list_of_guests[i].name} сел(-а) за стол номер {list_of_tables[i].number}')
            if len_list_of_guests > min_guests_tables:
                for i in range(min_guests_tables, len_list_of_guests):
                    self.queue.put(guests[i])
                    print(f'{list_of_guests[i].name} в очереди')
...
#     def discuss_guests(self, guest):
#         free_table = False
#         for table in self.tables:
#             if not table.is_busy:
#                 table.is_busy = True
#                 print(f'Посетитель {guest} номер сел за стол {table.number}')
#                 guest_thr = Guest(guest, self, self.queue, table)
#                 guest_thr.start()
#                 self.guest_threads.append(guest_thr)
#                 free_table = True
#                 return
#             if not free_table:
#                 print(f'Посетитель {guest} ждет стол')
